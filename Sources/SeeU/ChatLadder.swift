...
nonisolated final class ChatLadder {
    ...
    func render(maxPixelHeight: Int) -> CGImage? {
        guard maxPixelHeight > 0, let top, let bottom else { return nil }
        ...
        // 导出也有固定像素预算，防止合法输入累积后分配巨型位图。
        let pixelScale = (24_000_000 / (CGFloat(width) * CGFloat(fullHeight))).squareRoot()
        let scale = min(1, CGFloat(maxPixelHeight) / CGFloat(fullHeight), pixelScale)
