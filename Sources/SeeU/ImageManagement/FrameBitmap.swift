...
nonisolated final class FrameBitmap: @unchecked Sendable {
    ...
    convenience init(imageData: Data, limits: SeeUImageLimits) throws {
        ...
        guard width > 0, height > 0 else { throw SeeUImageError.invalidImage }
        guard width <= limits.maximumDimension, height <= limits.maximumDimension,
              width <= limits.maximumPixels / height else { throw SeeUImageError.pixelBudgetExceeded }
        // 当前算法坐标以直立截图为契约；拒绝带旋转标记的照片，避免错认原图坐标。
        let orientation = (properties[kCGImagePropertyOrientation] as? NSNumber)?.intValue ?? 1
        guard orientation == 1 else { throw SeeUImageError.unsupportedOrientation }
