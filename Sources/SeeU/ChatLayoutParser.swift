...
nonisolated struct ChatLayoutParser {
    ...
    func parse(
        ...
        excludedKeyboardTop: CGFloat? = nil,
        occluders: [CGRect] = []
    ) -> ParsedChatFrame {
        ...
        // 宿主 键盘顶部的标记比字母键更靠上，也包含候选栏；两者取更靠上的。
        let keyboardTop = [Self.detectKeyboardTop(lines, W: W, H: H), excludedKeyboardTop].compactMap { $0 }.min()
        ...
        for (index, group) in groups.enumerated() where !skip.contains(index) {
            ...
            // 宿主 画中画压住的气泡：按“显示不完整”处理，别让残缺文字进入对话列表。
            ...
            bubbles.append(ChatBubble(
                ...
                recognitionConfidence: group.map(\.confidence).min()
            ))
        }
